...
with admins as (
  ...
  weeks as (
    select distinct timestamp_trunc(up.sent_at, week) as week
  ...
  posts_per_member_per_week as (
    ...
    from team_week_combinations twc
    left join dbt_buffer.publish_updates up
      on twc.account_id = up.account_id
      and timestamp_trunc(up.sent_at, week) = twc.week
    group by 1, 2, 3
  )

select
  week,
  approx_quantiles(posts_count, 2)[offset(1)] as median_posts_per_member
from posts_per_member_per_week
group by week
order by week asc
`;